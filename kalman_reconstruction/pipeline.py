...
def run_Kalman_SEM_to_xarray(ds, 
                             state_variables, 
                             random_variables,
                             nb_iter_SEM=30, 
                             ...
                             suffix=""):
    """
    Run the Kalman SEM algorithm on the input dataset and return the results in an xarray Dataset.
    ...
    # ---------------
    (x_s, P_s, M, log_likelihod, x, x_f, Q,) = run_Kalman_SEM(
        y_list = y_list,
    ...
    print(np.shape(x_s))
    print(len(observation_variables))
    # ---------------
    ...
    # store M and Q
    result = result.assign_coords(dict(
...
def expand_dims_coords(ds1: xr.Dataset, ds2: xr.Dataset, select_dict: dict) -> xr.Dataset:
    """
...
#%%
# data = xr.load_dataset(r"C:\Users\Niebaum\Documents\Repositories\reconstruct-climate-indices\data\idealized_ocean\parameter_experiments\integrations.nc")
...
# #%%
# select_dict = dict(
#     ocean_restoring_timescale=365.25,
#     stochastic_forcing_intensity=0.0575,
#     ocean_oscillation_timescale=8766,
# )

# test = run_Kalman_SEM_to_xarray(ds = data.loc[select_dict],
#                         state_variables = ["surface_air_temperature", "sponge_sea_surface_temperature"], 
